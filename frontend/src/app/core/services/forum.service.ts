...
import { ForumThread, ForumPost, CreateThreadRequest, CreatePostRequest, PaginatedResponse } from '../../models/domain.model';
...
export class ForumService {
  ...
  getAllThreads(filter?: ForumFilterOptions): Observable<PaginatedResponse<ForumThread>> {
    ...
    return this.http.get<PaginatedResponse<ForumThread>>(`${this.baseUrl}/threads`, { params })
      .pipe(
        catchError(error => {
   ...
   */
  getThreadsByCategory(category: string, page = 0, size = 10): Observable<PaginatedResponse<ForumThread>> {
  ...
  getThreadPosts(threadId: string, page = 0, size = 10): Observable<PaginatedResponse<ForumPost>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    return this.http.get<PaginatedResponse<ForumPost>>(`${this.baseUrl}/threads/${threadId}/posts`, { params })
      .pipe(
        catchError(error => {
  ...
  getUserThreads(page = 0, size = 10): Observable<PaginatedResponse<ForumThread>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    return this.http.get<PaginatedResponse<ForumThread>>(`${this.baseUrl}/user/threads`, { params })
      .pipe(
        catchError(error => {
  ...
  getUserPosts(page = 0, size = 10): Observable<PaginatedResponse<ForumPost>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    return this.http.get<PaginatedResponse<ForumPost>>(`${this.baseUrl}/user/posts`, { params })
      .pipe(
        catchError(error => {
